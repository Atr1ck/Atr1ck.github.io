...
import { getCategories, listPictures } from "./api";
...
export default function PictureDashboard() {
  ...
  const categoryNames = new Map(categoriesQuery.data?.pictures.map((item) => [item.slug, item.name]) || []);
  const pictures = useMemo(() => {
    ...
    return (query.data?.pictures || []).filter((picture) => {
      ...
      const matchesCategory = category === "all" || picture.category === category;
  ...
  return (
    <main className="mx-auto w-full max-w-screen-2xl px-4 py-6 sm:px-6">
      ...
      <div className="mt-6 flex flex-col gap-3 border-y border-base-300 py-4 sm:flex-row sm:items-center">
        ...
        <select className="select select-sm rounded-md" aria-label="照片分类筛选" value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">全部分类</option>{categoriesQuery.data?.pictures.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}</select>
      </div>
      ...
      {!query.isLoading && !query.isError && (
        <div className="mt-4 overflow-x-auto border border-base-300 bg-base-100">
          <table className="table table-sm min-w-[780px]">
            <thead><tr><th>照片</th><th>分类</th><th>标签</th><th>日期</th><th>状态</th><th className="text-right">操作</th></tr></thead>
            <tbody>
              {pictures.map((picture) => <tr key={picture.id}>
                ...
                <td>{categoryNames.get(picture.category) || picture.category}</td>
              ...
              {pictures.length === 0 && <tr><td colSpan={6} className="py-10 text-center text-base-content/55">没有符合条件的照片</td></tr>}
            </tbody>
          </table>
        </div>
      )}
